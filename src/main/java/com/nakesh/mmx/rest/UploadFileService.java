...
public class UploadFileService {
	...
	public Response uploadFile(MultipartFormDataInput input) {
		...
		for (InputPart inputPart : inputParts) {
			...
			if (!validateFile(contentType)) {
				return Response.status(200).entity("file : "+ fileName + "  is not allowed to download because of Content-Type : "
								+ contentType + " is not allowed").build();
			}
	...
	@Path("/get")
	@Produces("image/text")
	public Response getFile(@DefaultValue("") @QueryParam("fileName") String _fileName, @DefaultValue("") @QueryParam("version") String _version) {
		...
		if (dataByte == null) {
			...
			response.header("Content-Disposition", "attachment; filename=" + _fileName);
		}
	...
	private boolean validateFile(String _contentType) {
		if (_contentType.contains("x-javascript")
				|| _contentType.contains("sql")
				|| _contentType.contains("octet-stream")
				|| _contentType.contains("exe")
				|| _contentType.contains("x-winexe")
				|| _contentType.contains("application/x-msdos-program")) {
			return false;
		}
		return true;
	}
	...
	private boolean isSameFile(String _inputFilePath, String _fileName,
			String _contentType, byte[] inputFileByte) {
